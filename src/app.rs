    ...
    fn update(&mut self, message: Self::Message) -> ShouldRender {
        match message {
            Msg::AddToCart(product) => {
                let cart_product = self.state.cart_products.iter_mut()
    ...
    fn view(&self) -> Html {
        ...
        let render = Router::render(move |switch: Route| match switch {
            Route::ProductDetail(id) => html! {
                <ProductDetail id=id on_add_to_cart=handle_add_to_cart.clone()/>
            },
            Route::HomePage => html! {
                <Home cart_products=cart_products.clone() on_add_to_cart=handle_add_to_cart.clone()/>
            },
        });
